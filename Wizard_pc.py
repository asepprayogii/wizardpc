...
<style>
    ...
    /* ── Tombol icon Ganti & Hapus ── */
    /* Kolom ke-2 (Ganti) → biru outlined */
    ...
    /* Kolom ke-3 (Hapus) → merah outlined */
...
def generate_bundles(df, branch_col, cat_col, price_min=0, price_max=0):
    ...
    modes = [
        {"key": "stok_terbanyak", "label": "Stok Terbanyak", "desc": "Produk dengan stok terbanyak dari setiap kategori",
         "badge": "badge-stock", "variants": ["#1 Stok Tertinggi", "#2 Stok Tinggi", "#3 Stok Cukup"]},
        {"key": "harga_termurah", "label": "Harga Termurah", "desc": "Produk dengan harga termurah dari setiap kategori",
         "badge": "badge-price", "variants": ["#1 Termurah", "#2 Budget", "#3 Ekonomis"]},
        {"key": "smart_pick", "label": "Smart Pick", "desc": "Harga termurah + range 100rb — stok terbanyak dalam range",
         "badge": "badge-smart", "variants": ["#1 Smart", "#2 Smart", "#3 Smart"]},
    ]
    ...
    for m in modes:
        ...
        for v_idx in range(3):
            ...
            if bundle:
                ...
                group["cards"].append({
                    "name": m["variants"][v_idx], "badge": m["badge"],
                    "in_range": in_range, **bundle
                })
        ...
        for group in grouped:
            ...
            for j, card in enumerate(cards):
                with cols[j]:
                    ...
                    note = "" if card["in_range"] else '<div style="font-size:11px;color:#e53935;margin-top:4px;">Di luar rentang harga</div>'
                    st.markdown(f"""
    ...
    # --------------------------------------------------------
        ...
        with col_parts:
            ...
            for cat in display_order:
                ...
                with c_ganti:
                    st.write("")
                    # ✏️ icon — aktif = X, nonaktif = pensil
                    icon_ganti = "✕" if st.session_state.ganti_cat == cat else "✏️"
                    if st.button(icon_ganti, key=f"ganti_{cat}", help=f"Ganti {cat}", use_container_width=True):
                        st.session_state.ganti_cat = None if st.session_state.ganti_cat == cat else cat
                        st.rerun()
                with c_hapus:
                    st.write("")
                    # 🗑️ icon hapus
                    if not st.button("🗑️", key=f"del_{cat}", help=f"Hapus {cat}", use_container_width=True):
